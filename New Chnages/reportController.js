...
exports.standardsRequisitionWiseReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Requisition_Report_${Date.now()}.xlsx`,
    );
    res.send(buffer);
  } catch (error) {
    next(error);
...
exports.standardsStatusWiseReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Status_Report_${Date.now()}.xlsx`,
    );
    res.send(buffer);
  } catch (error) {
    next(error);
...
exports.standardsNumberWiseReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Standard_Number_Report_${Date.now()}.xlsx`,
    );
    res.send(buffer);
  } catch (error) {
    next(error);
...
exports.standardsCompleteDirectoryReport = async (req, res, next) => {
  try {
    ...
    await buildStandardsSheet(workbook);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Complete_Standards_Directory_${Date.now()}.xlsx`,
    );
...
const buildStandardsSheet = async (workbook) => {
    ...
    periodicals.forEach((p) => {
      worksheet.addRow({
        subscriptionDate: p.subscriptionDate
          ? new Date(p.subscriptionDate).toLocaleDateString()
          : "N/A",
        title: p.title,
        subtitle: p.subtitle,
        ...
        receiptDate: p.receiptDate
          ? new Date(p.receiptDate).toLocaleDateString()
          : "",
        departmentToIssue: p.departmentToIssue,
        departmentIssueDate: p.departmentIssueDate
          ? new Date(p.departmentIssueDate).toLocaleDateString()
          : "",
...
exports.periodicalsFrequencyWiseReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Frequency_Report_${Date.now()}.xlsx`,
    );
    res.send(buffer);
  } catch (error) {
    next(error);
...
exports.periodicalsTitleWiseReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Title_Search_Report_${Date.now()}.xlsx`,
    );
    res.send(buffer);
  } catch (error) {
    next(error);
...
// 5. Missing Issues Report (Overdue Only)
exports.periodicalsMissingIssuesReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Missing_Issues_${Date.now()}.xlsx`,
    );
    res.send(buffer);
  } catch (error) {
    next(error);
...
// 6. Complete Directory (Comprehensive Data)
exports.periodicalsCompleteDirectoryReport = async (req, res, next) => {
  try {
    const workbook = new ExcelJS.Workbook();

    await buildPeriodicalsSheet(workbook);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Complete_Periodicals_Directory_${Date.now()}.xlsx`,
    );
...
const buildPeriodicalsSheet = async (workbook) => {
...
exports.abstractsSubjectWiseReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Subject_Report_${Date.now()}.xlsx`,
    );
...
exports.abstractsYearlyArchivesReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Yearly_Archive_${year || "All"}_${Date.now()}.xlsx`,
    );
...
exports.abstractsPublishedInAAReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Published_In_AA_${Date.now()}.xlsx`,
    );
...
exports.abstractsStatusWiseReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Abstracts_Status_Report_${Date.now()}.xlsx`,
    );
...
exports.abstractsCompleteDirectoryReport = async (req, res, next) => {
  try {
    ...
    await buildAbstractsSheet(workbook);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Complete_Abstract_Directory_${Date.now()}.xlsx`,
    );
...
const buildAbstractsSheet = async (workbook) => {
    ...
    members.forEach((m) => {
      const row = worksheet.addRow({
        ...
        membershipStartDate: m.membershipStartDate
          ? new Date(m.membershipStartDate).toLocaleDateString()
          : "",
...
exports.kcMembersOverdueReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=KC_Overdue_Report_${Date.now()}.xlsx`,
    );
...
exports.kcMembersCompleteReport = async (req, res, next) => {
  try {
    ...
    await buildKcMembersSheet(workbook);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Complete_KC_Members_${Date.now()}.xlsx`,
    );
...
const buildKcMembersSheet = async (workbook) => {
...
const createPDFTable = (doc, data, startY) => {
  ...
  data.forEach((item, index) => {
    ...
    doc.text(item.title || "N/A", 55, y + 8, {
      width: colWidths.title - 10,
      height: rowHeight - 16,
      ellipsis: true,
    });
    doc.text(item.type || "N/A", 235, y + 8, { width: colWidths.type - 10 });
    doc.text(item.category || "N/A", 335, y + 8, {
      width: colWidths.category - 10,
    });
    doc.text(item.itemType || "N/A", 425, y + 8, {
      width: colWidths.itemType - 10,
    });
    doc.text(item.status || "N/A", 505, y + 8, {
...
exports.arrivalsNewsCategoryWiseReport = async (req, res, next) => {
  try {
    ...
    const items = await ArrivalAndNews.find({ category }).sort({
      createdAt: -1,
    });
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Arrivals_News_Category_${category}_${Date.now()}.pdf`,
    );
...
exports.arrivalsNewsTypeWiseReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Arrivals_News_Type_${type.replace(/ /g, "_")}_${Date.now()}.pdf`,
    );
...
exports.arrivalsNewsPriorityWiseReport = async (req, res, next) => {
  try {
    ...
    const items = await ArrivalAndNews.find({ priority }).sort({
      createdAt: -1,
    });
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Arrivals_News_Priority_${priority}_${Date.now()}.pdf`,
    );
...
exports.arrivalsNewsStatusWiseReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Arrivals_News_Status_${status}_${Date.now()}.pdf`,
    );
...
exports.arrivalsNewsCompleteReport = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Arrivals_News_Complete_Report_${Date.now()}.pdf`,
    );
...
//STATUS WISE
exports.ajmtStatusWiseReport = async (req, res, next) => {
  try {
    const { status } = req.query;
    ...
    const papers = await AJMTPaper.find(filter).sort({
      status: 1,
      paperTitle: 1,
    });

    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("AJMT Status Report");

    worksheet.columns = [
      { header: "Unique ID", key: "uniqueId", width: 20 },
      { header: "Paper Title", key: "paperTitle", width: 40 },
      { header: "Title Subject", key: "titleSubject", width: 25 },
      { header: "Paper Type", key: "paperType", width: 20 },
      { header: "Status", key: "status", width: 15 },

      { header: "Submission Date", key: "date", width: 18 },
      {
        header: "Tentative Publication Date",
        key: "tentativeDateOfPublication",
        width: 22,
      },
      {
        header: "Website Update Date",
        key: "websiteUpdateDate",
        width: 18,
      },
      { header: "Hardcopy Date", key: "hardcopyDate", width: 18 },
      { header: "Review Date", key: "reviewDate", width: 18 },
      { header: "Paper File Path", key: "paperFile", width: 40 },
      { header: "Paper File Size (Bytes)", key: "paperFileSize", width: 20 },
      { header: "Paper Upload Date", key: "paperUploadDate", width: 20 },
      {
        header: "Manuscript Received Date",
        key: "manuscriptReceivedDate",
        width: 20,
      },

      { header: "Plagiarism %", key: "plagiarismPercentage", width: 15 },
      { header: "AI Plagiarism %", key: "aiPlagiarismPercentage", width: 18 },
      { header: "Plagiarism Remarks", key: "plagiarismRemarks", width: 25 },

      { header: "Copyright Received", key: "copyrightReceived", width: 20 },

      { header: "Authors", key: "authors", width: 60 },
    ...
    papers.forEach((paper) => {
      const authorsText = (paper.authors || [])
        .map(
          (a) =>
            `Name: ${a.authorName} Email: ${a.authorEmail || ""} Institution: ${a.authorInstitution || ""} Address: ${a.authorAddress || ""} City: ${a.authorCity || ""} Country: ${a.authorCountry || ""} Phone: ${a.authorPhone || ""}`,
        )
        .join("\n\n");
      ...
      worksheet.addRow({
        ...
        date: paper.date ? new Date(paper.date).toLocaleDateString() : "",

        tentativeDateOfPublication: paper.tentativeDateOfPublication
          ? new Date(paper.tentativeDateOfPublication).toLocaleDateString()
          : "",

        websiteUpdateDate: paper.websiteUpdateDate
          ? new Date(paper.websiteUpdateDate).toLocaleDateString()
          : "",

        hardcopyDate: paper.hardcopyDate
          ? new Date(paper.hardcopyDate).toLocaleDateString()
          : "",

        manuscriptReceivedDate: paper.manuscriptReceivedDate
          ? new Date(paper.manuscriptReceivedDate).toLocaleDateString()
          : "",

        plagiarismPercentage: paper.plagiarismPercentage,
        aiPlagiarismPercentage: paper.aiPlagiarismPercentage,
        ...
        reviewDate: paper.reviewDate
          ? new Date(paper.reviewDate).toLocaleDateString()
          : "",

        paperFile: paper.paperFile || "",

        paperFileSize: paper.paperFileSize || "",

        paperUploadDate: paper.paperUploadDate
          ? new Date(paper.paperUploadDate).toLocaleDateString()
          : "",

        remarks: paper.remarks,

        paperFileName: paper.paperFileName,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=AJMT_${status || "All"}_${Date.now()}.xlsx`,
    );
...
// COMPLETE REPORT
exports.ajmtReport = async (req, res, next) => {
  try {
    const workbook = new ExcelJS.Workbook();

    await buildajmtReportSheet(workbook);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Complete_AJMT_Reports_${Date.now()}.xlsx`,
    );
...
const buildajmtReportSheet = async (workbook) => {
  const papers = await AJMTPaper.find().sort({
    status: 1,
    paperTitle: 1,
  });
...
exports.completeBackupReport = async (req, res, next) => {
  try {
    const workbook = new ExcelJS.Workbook();
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Backup_Sheet_${Date.now()}.xlsx`,
    );
