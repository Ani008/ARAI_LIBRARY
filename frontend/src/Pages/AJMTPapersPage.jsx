...
const AJMTPapersPage = () => {
  ...
  const fetchPapers = async () => {
    ...
    try {
      ...
      const response = await api.get("/ajmtpapers", {
        params: {
  ...
  const handleDelete = async (paperId) => {
    ...
    try {
      const response = await api.delete(`/ajmtpapers/${paperId}`);
  ...
  return (
    <PermissionGuard
      module="ajmtPapers"
      fallback={<AccessDeniedOverlay title="AJMT Research Papers" />}
    >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-middle">
                      ...
                      {paper.tentativeDateOfPublication && (
                        <div className="text-xs text-cyan-800 mt-1">
                          Tentative:{" "}
                          {formatDate(paper.tentativeDateOfPublication)}
                        </div>
                      )}
                    </td>
