...
const EmailPreviewModal = ({
  ...
  const [emailSubject, setEmailSubject] = useState(`Research Paper Review Request: ${paperTitle}`);
  ...
  const tableStyle = "border-collapse: collapse; width: 100%; border: 1px solid black;";
  ...
  const beforeEditable = `
    <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; color: #000;">
      ...
      <table border="1" cellpadding="0" cellspacing="0" style="${tableStyle}">
        ...
        <tr><td style="${tdStyle}"><strong>Date Received from Author</strong></td><td style="${tdStyle}">${manuscriptReceivedDate}</td></tr>
        <tr><td style="${tdStyle}"><strong>Plagiarism %</strong></td><td style="${tdStyle}">${plagiarismPercentage || ""}</td></tr>
        <tr><td style="${tdStyle}"><strong>Expected Date of Review Report:</strong></td><td style="${tdStyle}"></td></tr>
      </table>
      <br/>
  `;
  ...
  const afterEditable = `
      <br/>
      <p><strong>RATING SCORE</strong></p>
      <table border="1" cellpadding="0" cellspacing="0" style="${tableStyle} text-align: left;">
        <tr>
          <th style="${tdStyle} width: 10%;">Sl. No.</th>
          <th style="${tdStyle} width: 60%;">Particulars</th>
          <th style="${tdStyle} width: 30%;">Score (1-10)</th>
        </tr>
      ...
      <p style="font-size: 13px;">Scores range from 1 to 10, with 1 being the lowest and 10 being the highest.</p>
      <br/>
      <p><strong>Whether Recommended for Journal (Please √ mark)</strong></p>
      <table border="1" cellpadding="0" cellspacing="0" style="${tableStyle}">
        <tr>
          <td style="${tdStyle}">Approved</td><td style="${tdStyle} width: 40px;"></td>
          <td style="${tdStyle}">Approved if Modified-Minor</td><td style="${tdStyle} width: 40px;"></td>
          <td style="${tdStyle}">Approved if Modified – Major</td><td style="${tdStyle} width: 40px;"></td>
          <td style="${tdStyle}">Disapproved</td><td style="${tdStyle} width: 40px;"></td>
        </tr>
      </table>
      <br/>
      <p><strong>Comments & Corrective Actions: (Why it is Approved / Disapproved? What corrections are required?)</strong></p>
      <table border="1" cellpadding="0" cellspacing="0" style="${tableStyle}">
      ...
      <br/>
      <p>Yours Sincerely,</p>
      <p><strong>Dr A. Madhava Rao</strong><br/>
      ...
      Pune - Maharashtra<br/>
      Tel: 020-6762-1126 | https://araijournal.com</p>
    </div>
  `;
  ...
  useEffect(() => {
    if (isOpen && editorRef.current && !quillRef.current) {
      ...
      quillRef.current.on("text-change", () => setEditableContent(quillRef.current.root.innerHTML));
    }
  }, [isOpen, reviewer]);
  ...
  const handleSend = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/ajmtpapers/${paperId}/send-email/${reviewer.reviewerNumber}`, {
        method: "POST",
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
      <div className="bg-white rounded-lg shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          {sent && <div className="text-green-600 bg-green-50 p-2 rounded">Email Sent!</div>}
          <input 
          ...
          <div className="border p-4 bg-gray-50 rounded">
            <div dangerouslySetInnerHTML={{ __html: beforeEditable + editableContent + afterEditable }} />
          </div>
        </div>
        <div className="p-4 border-t bg-gray-50 flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 border rounded">Cancel</button>
          <button onClick={handleSend} disabled={sending} className="bg-emerald-600 text-white px-6 py-2 rounded flex items-center">
