...
const Sidebar = () => {
  ...
  const menuGroups = [
    ...
    },
    {
      title: "NEWS & UPDATES",
      items: [
        { name: 'New Arrivals & News', icon: NotepadTextDashedIcon, path: '/new-arrivals-news' },
      ]
    }
  ];
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <item.icon className={`mr-3 h-5 w-5 ${isActive ? 'text-indigo-500' : 'text-gray-400 group-hover:text-gray-600'}`} />
                    {item.name}
                  </button>
                );
              })}
